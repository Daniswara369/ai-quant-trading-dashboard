...
export default function AgentThoughts({ symbol, timeframe }) {
    ...
    wsRef.current.onmessage = (event) => {
      try {
        ...
        if (data.type === 'info') {
          ...
          setLoading(false);
          // Re-fetch committee snapshot so weights update after each analysis
  ...
  const fetchCommitteeSnapshot = async () => {
    try {
      const res = await fetch('/api/agent-committee/snapshot');
      if (res.ok) {
        const data = await res.json();
        setCommitteeStats(data);
      }
    } catch (err) {
      console.error("Failed to fetch committee snapshot", err);
    }
  };
  ...
  useEffect(() => {
    ...
    // Run every 15 minutes (900,000 ms). We click the physical button to avoid state closure bugs.
  ...
  const toggleAgentExpansion = (name) => {
    ...
    <div className="metric-card p-4 space-y-4">
      <div className="flex items-center justify-between border-b border-slate-700/50 pb-3">
        <div className="flex items-center gap-3">
          ...
          {committeeStats && (
            <span className="bg-slate-800 text-slate-400 text-[10px] px-2 py-0.5 rounded border border-slate-700" title="Historical decisions tracked for performance weighting">
              {committeeStats.n_log_rows} records tracked
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded transition-all font-bold flex items-center gap-1 border ${
              isAutoPilot 
                ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/50 shadow-[0_0_10px_rgba(16,185,129,0.2)]' 
                : 'bg-slate-800 hover:bg-slate-700 text-slate-400 border-slate-700'
            }`}
          >
            {isAutoPilot ? '🟢 Auto-Pilot (15m)' : '⚪ Auto-Pilot OFF'}
          </button>
          <button
            id="btn-run-analysis"
            onClick={runAnalysis}
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
            ...
            {consensus && consensus.agent_outputs.map((agent, idx) => {
              const weightValue = committeeStats?.weights?.[agent.agent_name];
              const weightPct = weightValue ? (weightValue * 100).toFixed(1) : '—';

              const perfNode = committeeStats?.performance?.find(p => p.agent_name === agent.agent_name);
              const winRate = perfNode ? (perfNode.ema_win_15m * 100).toFixed(1) : '—';
              const isExpanded = expandedAgents[agent.agent_name];

              return (
                <div key={idx} className="bg-slate-800/60 rounded p-2 border border-slate-700 flex flex-col gap-1.5">
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-xs text-slate-200">
                      {getAgentIcon(agent.agent_name)} {agent.agent_name}
                    </span>
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${agent.signal === 'BUY' ? 'bg-emerald-500/20 text-emerald-400' :
                  ...
                  <div className="flex justify-between items-center text-[10px] text-slate-500 border-b border-slate-700/50 pb-1.5 mb-1">
                    <div className="flex gap-3">
                      <span title="Algorithmically assigned weight based on historical performance">
                        Weight: <span className="text-blue-400 font-medium">{weightPct}%</span>
                      </span>
                      <span title="15m rolling Exponential Moving Average win-rate">
                        Win Rate: <span className="text-emerald-400 font-medium">{winRate}%</span>
                      </span>
                    </div>
                    <button 
                      onClick={() => toggleAgentExpansion(agent.agent_name)}
                      className="text-[9px] uppercase tracking-wider font-bold text-blue-500 hover:text-blue-400 transition-colors"
                    >
                  ...
                  {isExpanded && (
                    <div className="text-[10px] text-slate-400 leading-tight bg-slate-900/40 p-2 rounded border border-slate-800 animate-in fade-in slide-in-from-top-1 duration-200">
                      {agent.reasoning}
                    </div>
                  )}
                </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs text-slate-400 uppercase tracking-widest font-semibold flex items-center gap-2">
                Manager Consensus
                <span className={`text-xs px-2 py-0.5 rounded font-bold ${consensus.signal === 'BUY' ? 'bg-emerald-500/20 text-emerald-400' :
