...
1.  **15-Minute Markout Resolution**: Every signal generated by an agent is tracked and "graded" against the market price after a 15-minute window.
2.  **EMA-Based Performance Tracking**: We calculate a rolling Exponential Moving Average (EMA) of each agent's prediction accuracy.
3.  **Variable Influence**: Agents who are on a "winning streak" automatically receive a higher Weighting in the Manager's final consensus decision. This prevents any single agent's failure from compromising the entire system.
